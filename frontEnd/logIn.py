...
class login(customtkinter.CTk):
    ...
    def checkpassword(self):
        ...
        if len(data) > 0:
            ...
            self.attempts -= 1
            if self.attempts == 0:
                messagebox.showwarning("Blood Donation System",
                                       'Your attempts are exhausted! \nThe program is now closing.')
                self.destroy()
            else:
                messagebox.showwarning("Blood Donation System",
                                       f"Incorrect password or username! \n{self.attempts} attempts left of 3.\nThe "
                                       f"program will close after 3 failed attempts.")

        # pass

    def close(self):
        response = messagebox.askyesno("Blood Donation System", "Are you sure you wish to exit?")
        if response:
            self.destroy()
